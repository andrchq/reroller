...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4">
      <Card className="w-full max-w-md border-white/15 p-6">
        <div className="mb-6">
          <div className="mb-2 text-3xl font-semibold tracking-tight text-white">Reroller</div>
          <p className="text-sm text-[var(--muted)]">
            {hasUsers ? "Войдите в приватную панель." : "Создайте первого администратора."}
          </p>
        </div>
        {params.error ? <div className="mb-4 rounded-xl border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-200">Неверный логин или пароль.</div> : null}
